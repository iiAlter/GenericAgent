...
''' + get_global_memory()
    ...
    def _sidebrain_client(self):
        """懒加载 sidebrain MCP client"""
        if not hasattr(self, '_sbc'):
            try:
                ...
                # fallback: 启动 MCP server 子进程
                import subprocess, sys
                from pathlib import Path
                server_path = Path.home() / '.pi' / 'agent' / 'extensions' / 'sidebrain-mcp-server.mjs'
                if not server_path.exists():
                    raise RuntimeError('sidebrain MCP server not found')
                proc = subprocess.Popen(
                    ['node', str(server_path)],
                    stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE, text=True,
                )
                self._sbc = {'_proc': proc}
                raise  # 内部异常，下次重试走 mcp_client
        return self._sbc
    ...
    def do_sidebrain_search(self, args, response):
        ...
        try:
            client = self._sidebrain_client()
            result = client['search'](query, limit)
        except Exception as e:
            result = f'Sidebrain 搜索失败: {e}\n提示: 检查 sidebrain MCP server 是否运行'
    ...
    def do_sidebrain_get(self, args, response):
        ...
        try:
            client = self._sidebrain_client()
            result = client['get_detail'](topic_id)
            if not result:
                result = f'未找到: {topic_id}'
        except Exception as e:
            result = f'Sidebrain 获取失败: {e}'
        return StepOutcome(result or '未找到', next_prompt='\n')

    def do_sidebrain_ingest(self, args, response):
        """将信息存入 sidebrain 知识库"""
        text = args.get('text', '')
        source = args.get('source', 'ga')
        try:
            client = self._sidebrain_client()
            result = client['ingest'](text, source)
            result_text = result.get('text', str(result))
        except Exception as e:
            result_text = f'Sidebrain 摄入失败: {e}'
        return StepOutcome(result_text, next_prompt='\n')
