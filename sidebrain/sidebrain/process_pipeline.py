    ...
    for raw_dir, source_type in raw_dirs:
        ...
        for f in files:
            ...
            prompt = (
                ...
                f"3. 按 L1-L4 记忆层级给每条记忆分级\n"
                f"4. 用 file_write 将结果写入 JSON 文件: {json_output_path}\n\n"
                f"记忆层级（L1-L4）：\n"
                f"  L1 索引层：高频场景关键词 → 记忆定位。≤30 行全局索引，仅存指针不存细节。\n"
                f"  L2 事实层：环境事实（路径、配置、端口、凭证、API 端点等），不因时间变化。\n"
                f"  L3 任务层：特定任务的 SOP、避坑指南、修复方案、脚本等可供复用的知识。\n"
                f"  L4 会话层：原始会话摘要，用于定位历史上下文。\n\n"
                f"分级规则：\n"
                f"  - 这是环境特异性事实（非标路径、配置、端口等）？→ L2\n"
                f"  - 这是特定任务的技术知识（艰难尝试才成功的方案）？→ L3\n"
                f"  - 这是高频使用的索引/指针（场景关键词）？→ L1\n"
                f"  - 这是会话摘要/上下文引用？→ L4\n"
                f"  - 通用常识 → 不记录\n\n"
                f"JSON 格式：\n"
                f"{{\n"
